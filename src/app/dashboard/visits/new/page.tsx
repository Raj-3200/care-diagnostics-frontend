...
export default function NewVisitPage() {
  ...
  const { data: patients } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<ApiResponse<Patient[]>>(`/patients?search=${patientSearch}&limit=10`);
      return data.data;
    },
            ...
            <CardContent className="pt-6">
              <div className="mb-5 flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-muted-foreground">Search and select the patient for this visit</p>
                </div>
              </div>
              ...
              {selectedPatient ? (
                <div className="flex items-center justify-between rounded-lg border border-primary/20 bg-primary/[0.03] p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-foreground">{selectedPatient.firstName} {selectedPatient.lastName}</p>
                      <p className="text-[12px] text-muted-foreground">MRN: {selectedPatient.mrn} &bull; {selectedPatient.phone}</p>
                    </div>
                  </div>
                  <Button type="button" variant="ghost" size="sm" onClick={() => { setSelectedPatientId(''); setPatientSearch(''); }} className="h-8 gap-1.5 rounded-lg text-[12px] text-muted-foreground hover:text-foreground">
                    ...
                    <div className="rounded-lg border border-border/60 overflow-hidden">
                      {patients.map((p) => (
                        ...
                        >
                          ...
                          <div>
                            <p className="text-[13.5px] font-medium">{p.firstName} {p.lastName}</p>
                            <p className="text-[12px] text-muted-foreground">MRN: {p.mrn} &bull; {p.phone}</p>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <div className="mb-5 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[12px] text-muted-foreground">Choose diagnostic tests to order</p>
                  </div>
                </div>
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-0.5 text-[12px] text-muted-foreground">
                          <span className="font-mono">{test.code}</span> &bull; ₹{Number(test.price).toFixed(0)}
                        </p>
                      </div>
                    </label>
                  );
